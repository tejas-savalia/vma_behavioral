...
def fit_single(participant):
    ...
    try:
        ...
        errors = data.loc[data['p_id'] == participant, 'init errors'].values[64:]
...
def fit_dual(participant):
    single_fits = pd.read_csv('model_results/single_fit_initerror_washout_results.csv')
    print('participant started: ', participant)
...
def fit_dual_cv(participant, errors, p_type, train_indices, test_indices):
    # print('participant started: ', participant)
    dual_fits = pd.read_csv('model_results/dual_fit_initerror_washout_results.csv')
...
if __name__ == '__main__':
    ...
    df.to_csv('model_results/single_fit_initerror_washout_results.csv')
    dual_fit_results = pool.map(fit_dual, participant)
    
    df = pd.DataFrame(dual_fit_results, columns =['p_id', 'gof', 'As', 'Bs', 'Af', 'Bf', 'Eps'])
    
    df.to_csv('model_results/dual_fit_initerror_washout_results.csv')
    ...
    df_full_single.to_csv('model_results/single_fit_initerror_washout_results_cv.csv', index = False)

    df_full_dual = pd.concat(dual_fit_df)
    df_full_dual.to_csv('model_results/dual_fit_initerror_washout_results_cv.csv', index=False)
